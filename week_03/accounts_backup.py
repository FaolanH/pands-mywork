...
# Immeditely lists came to mind for this, with the feature to replace positions in the list e.g. 
# the first 6 numbers to be replaced with X, still not figured out how to ensure the input is int only 
#This slack overflow https://stackoverflow.com/questions/1277914/is-there-a-way-to-output-the-numbers-only-from-a-python-list
#maybe use boolean?
...
# I had previously done this with an if loop statement instead of a while loop, and noticed that it restarts
# the whole run and does not allow user to try input again, so a while loop makes more sense
